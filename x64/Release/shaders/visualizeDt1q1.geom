...
void drawApparentRidgeSegment(const int v0,const int v1,const int v2,
    ...
    if (to_center) {
      ...
      p12 = (geometryIn[v0].pos + geometryIn[v1].pos + geometryIn[v2].pos) / 3.0;
   ...
   if (do_test) {
      ...
      // We want tmax1 to point opposite to perp, and tmax0 and tmax2 to point along it.  Otherwise, return.
...
void main() {
    if(cull){
        for(int i =0;i<3;i++){
            if(geometryIn[i].normalDotView<=-0.5) return;
        }
    }
   ...
   // emax are derivatives of max curvatures in max direction
    ...
    vec3 world_t1_0 = geometryIn[0].t1[0] * geometryIn[0].maxPrincipal +
                      geometryIn[0].t1[1] * geometryIn[0].minPrincipal;
    vec3 world_t1_1 = geometryIn[1].t1[0] * geometryIn[1].maxPrincipal +
                      geometryIn[1].t1[1] * geometryIn[1].minPrincipal;
    vec3 world_t1_2 = geometryIn[2].t1[0] * geometryIn[2].maxPrincipal +
